...
class DocumentServiceTest {
    ...
    @Test
    void getDocument() {
        when(client.get(any(String.class), any())).thenReturn(new HttpResponse(null, 200, "{\"id\":25684,\"title\":\"American Ninja 5\",\"poster\":\"https://image.tmdb.org/t/p/w1280/iuAQVI4mvjI83wnirpD8GVNRVuY.jpg\",\"overview\":\"When a scientists daughter is kidnapped, American Ninja, attempts to find her, but this time he teams up with a youngster he has trained in the ways of the ninja.\",\"release_date\":\"1993-01-01\"}"));
    ...
    @Test
    void getDocuments() {
        when(client.get(any(String.class), any())).thenReturn(new HttpResponse(null, 200, "[{\"id\":25684,\"release_date\":\"1993-01-01\",\"poster\":\"https://image.tmdb.org/t/p/w1280/iuAQVI4mvjI83wnirpD8GVNRVuY.jpg\",\"title\":\"American Ninja 5\",\"overview\":\"When a scientists daughter is kidnapped, American Ninja, attempts to find her, but this time he teams up with a youngster he has trained in the ways of the ninja.\"},{\"id\":468219,\"title\":\"Dead in a Week (Or Your Money Back)\",\"release_date\":\"2018-09-12\",\"poster\":\"https://image.tmdb.org/t/p/w1280/f4ANVEuEaGy2oP5M0Y2P1dwxUNn.jpg\",\"overview\":\"William has failed to kill himself so many times that he outsources his suicide to aging assassin Leslie. But with the contract signed and death assured within a week (or his money back), William suddenly discovers reasons to live... However Leslie is under pressure from his boss to make sure the contract is completed.\"}]"));
    ...
    void addAndReplaceDocument() {
        when(client.post(any(String.class), any())).thenReturn(new HttpResponse(null, 200, "{\"updateId\":1}"));
    ...
    void addAndUpdateDocument() {
        when(client.put(any(String.class), any(), any())).thenReturn(new HttpResponse(null, 200, "{\"updateId\":1}"));
    ...
    @Test
    void deleteDocument() {
        when(client.delete(any(String.class))).thenReturn(new HttpResponse(null, 200, "{\"updateId\": 1}"));
        assertEquals(1, classToTest.deleteDocument("123").getUpdateId());
        when(client.delete(any(String.class))).thenReturn(new HttpResponse(null, 200, null));
        assertThrows(MeiliJSONException.class, () -> classToTest.deleteDocument("123"));
        when(client.delete(any(String.class))).thenReturn(new HttpResponse(null, 200, ""));
        assertThrows(MeiliJSONException.class, () -> classToTest.deleteDocument("123"));
    }
    ...
    @Test
    void search() {
        when(client.post(any(String.class), any())).thenReturn(new HttpResponse(null, 200, "{\"hits\":[{\"id\":\"2770\",\"title\":\"American Pie 2\",\"poster\":\"https://image.tmdb.org/t/p/w1280/q4LNgUnRfltxzp3gf1MAGiK5LhV.jpg\",\"overview\":\"The whole gang are back and as close as ever. They decide to get even closer by spending the summer together at a beach house. They decide to hold the biggest...\",\"release_date\":997405200},{\"id\":\"190859\",\"title\":\"American Sniper\",\"poster\":\"https://image.tmdb.org/t/p/w1280/svPHnYE7N5NAGO49dBmRhq0vDQ3.jpg\",\"overview\":\"U.S. Navy SEAL Chris Kyle takes his sole mission—protect his comrades—to heart and becomes one of the most lethal snipers in American history. His pinpoint accuracy not only saves countless lives but also makes him a prime...\",\"release_date\":1418256000}],\"offset\":0,\"limit\":20,\"nbHits\":1,\"exhaustiveNbHits\":false,\"processingTimeMs\":2,\"query\":\"test120232\"}"));
    ...
    @Test
    void getUpdate() {
        when(client.get(any(String.class), any())).thenReturn(new HttpResponse(null, 200, "{\"status\":\"processed\",\"updateId\":1,\"type\":{\"name\":\"DocumentsAddition\",\"number\":4},\"duration\":0.076980613,\"enqueuedAt\":\"2019-12-07T21:16:09.623944Z\",\"processedAt\":\"2019-12-07T21:16:09.703509Z\"}"));
