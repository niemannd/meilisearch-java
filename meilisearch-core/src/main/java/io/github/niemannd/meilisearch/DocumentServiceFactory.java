...
public class DocumentServiceFactory {
     ...
     */
    public <T> DocumentService<T> createService(String indexName, Configuration config, ServiceTemplate serviceTemplate) {
        return new DocumentService<>(indexName, config, serviceTemplate);
    }
}
